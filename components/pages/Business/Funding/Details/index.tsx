...
// ants and icons
import { Typography, Breadcrumb, Avatar, Skeleton, Spin } from "antd";
...
const FundingDetails = () => {
  ...
  const fetchFundingDetails = async () => {
    try {
      ...
      console.log("Unable to fetch fundings", error);
  ...
  const fetchApplicantList = async () => {
    try {
      ...
      const res = await axiosInstance.get(
        `/funding/business-funding-applicants/${fundingId}${query}`
      );
      ...
      console.log("Unable to fech applicants", error);
      // handleAxiosError(error);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    fetchApplicantList();
  }, [fundingId, searchTerms, country, experienceLevel]);
        ...
        <div>
          <section className="content-header">
            <Container fluid className="p-6">
              ...
              <Breadcrumb
                items={[
                  { title: "Fundings", href: "/business/funding" },
                  { title: "Applicants" },
                ]}
                separator=">"
              />
          ...
          <Skeleton active loading={fundingLoading} className="p-4">
