...
const ReviewPost = ({
  ...
  const handleMakePayment = async () => {
    const paymentEndpoint =
      coursePaymentBody && userCountry == "Nigeria"
        ? "/ng-payment/manage/free-course-card-otp-business"
        : "/payment/manage/free-course-payment";
    setLoading(true);
    try {
      const payForCourse = await axiosInstance.post(`${paymentEndpoint}`, {
        ...coursePaymentBody,
      });
      if (payForCourse) {
        ...
        setIsModalOpen(true);
        setTimeout(function () {
          router.push("/user/manage-access/course");
        }, 2000);
      }
    } catch (error: any) {
  ...
  useEffect(() => {
    ...
    if (userCountry) {
      dispatch(fetchCountryDiscount({ type: "course", country: userCountry }));
    }
    if (businessId) {
      dispatch(fetchBusinessDiscount({ type: "course", businessId }));
    }
  }, []);
