...
const CreateCertificateForm = () => {
  ...
  const handleSaveTemplate = async () => {
    ...
    if (imageUrl) {
      metaData.background = imageUrl;
    }
    const metaString = JSON.stringify(metaData);
    try {
      setLoading(true);
      const res = await axiosAccessInstance.post("/course-certificate", {
      ...
      if (res.status) {
        ...
        console.log("here is how to code", res);
      }
    } catch (error) {
  ...
  const handleDeleteCertificate = async () => {
    try {
      const res = await axiosAccessInstance.delete(
        `/course-certificate/course/${certificateTemplates.id}`
      );
      if (res.status) {
              ...
              <Form layout="vertical" size="large">
                ...
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Please Certificate of Completion",
                    },
                  ]}
                >
                  <Input size="large" placeholder="Certificate of Completion" />
                </Form.Item>
                <Form.Item
                  name="presentedTo"
                  label="Field 2"
                  rules={[
                    {
                      required: true,
                      message: "Please enter Presented to",
                    },
                  ]}
                >
                  <Input placeholder="This Certificate is Presented to" />
                </Form.Item>
                <Form.Item
                  name="studentName"
                  label="Field 3"
                ...
                >
                  <Input placeholder="Who has successfully Completed the requirements" />
                </Form.Item>
                <Form.Item
                  name="courseTitle"
                  label="Field 5"
                  rules={[
                    {
                      required: true,
                      message: "Please Fill course Title",
                    },
                  ]}
                >
                  <Input placeholder="{{ Course_Title }}" />
                </Form.Item>
                <Form.Item
                  name="offeredBy"
                  label="Field 6"
