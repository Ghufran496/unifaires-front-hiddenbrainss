...
const ReviewPost = ({
  ...
  const businessId = session?.user.businessAccess.ownersId;
  ...
  const handleMakePayment = async () => {
    const paymentEndpoint =
      fundingPayment && fundingPayment?.country == "Nigeria"
        ? "/ng-payment/manage/funding-card-otp"
        : "/payment/manage/funding";
    setLoading(true);
    try {
      const payForFunding = await axiosAccessInstance.post(
        `${paymentEndpoint}`,
        {
          ...fundingPayment,
        }
      );
      if (payForFunding.status) {
        // message.success("Payment made Successfully");
        showSuccess("Funding Payment Sucessful");
        setIsModalOpen(true);
        setTimeout(function () {
          router.push("/user/business-access/funding");
        }, 2000);
      }
    } catch (error: any) {
  ...
  useEffect(() => {
    ...
    if (userCountry) {
      dispatch(fetchCountryDiscount({ type: "funding", country: userCountry }));
    }
    if (businessId) {
      dispatch(fetchBusinessDiscount({ type: "funding", businessId }));
    }
  }, []);
