...
const FundingCategories = ({ prev, requestBody, next }: any) => {
  ...
  const handleFinish = async () => {
    try {
      ...
      const res = await axiosAccessInstance.post("/manage-funding", inputdata);

      if (res?.data?.data?.id) {
        toast.success("Funding Saved Successfully");
        router.push("/user/business-access/funding/create/" + res.data.data.id);
      } else {
        toast.error("Validation Error. Select a Category");
      }
    } catch (error) {
      handleAxiosError(error);
    } finally {
    ...
    <div>
      {FundingId && (
        <div className=" mb-8">
          ...
          {typeof fundingData?.data?.fundingCategory?.name === "string" &&
            fundingData.data.fundingCategory.name.trim() !== "" && (
              <Tag className="text-[#5832DA] ml-6 bg-[#D2C5FD] rounded-full border-none px-4 py-1">
                {fundingData.data.fundingCategory.name}
              </Tag>
            )}
        </div>
      )}
